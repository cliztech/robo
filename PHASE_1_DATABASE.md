...
Go to **Storage → Create Bucket** and use:
...
Go to **Storage → tracks → Policies** and create:
...
export async function GET() {
  try {
    ...
    // Test connection
    const { error } = await supabase.from('stations').select('count').limit(1)
...
- Go to **Database → Backups → Settings**
