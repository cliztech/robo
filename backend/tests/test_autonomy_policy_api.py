        ...
        try:
            ...
            show_payload = {
                **baseline,
                ...
                "show_overrides": [
                    {"show_id": "show-1", "mode": "auto_with_human_override"}
                ],
                "timeslot_overrides": [],
            }

            put_response = client.put(
                "/api/v1/autonomy-policy", json=show_payload, headers=headers
            )
            ...
            put_response = client.put(
                "/api/v1/autonomy-policy", json=timeslot_payload, headers=headers
            )
            assert put_response.status_code == 200
    ...
    with unittest.mock.patch.dict(os.environ, {"ROBODJ_SECRET_KEY": "test-secret-key"}):
        ...
        try:
            invalid_policy = {
                "station_default_mode": "manual_assist",
                "mode_permissions": {
                    "manual_assist": {"track_selection": "human_only"},
                    "semi_auto": {
