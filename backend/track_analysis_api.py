...
router = APIRouter(
    prefix="/api/v1/ai", tags=["track-analysis"], dependencies=[Depends(verify_api_key)]
)
...
def analyze_track(
    ...
    try:
        ...
        return TrackAnalysisEnvelope(
            status="success", success=True, data=result, error=None
        )
    except (ValidationError, ValueError, TypeError) as exc:
        return TrackAnalysisEnvelope(
            status="degraded", success=True, data=None, error=str(exc)
        )
    except (TimeoutError, AITimeoutError) as exc:
        envelope = TrackAnalysisEnvelope(
            status="failed", success=False, data=None, error=str(exc)
        )
