...
DEFAULT_MODE_PERMISSIONS: Dict[GlobalMode, PermissionMatrix] = {
    ...
    GlobalMode.assisted: {
        DecisionType.track_selection: DecisionAuthority.human_with_ai_assist,
        DecisionType.script_generation: DecisionAuthority.ai_with_human_approval,
        DecisionType.voice_persona_selection: DecisionAuthority.human_with_ai_assist,
...
class AutonomyPolicy(BaseModel):
    station_default_mode: GlobalMode = GlobalMode.assisted
    mode_permissions: Dict[GlobalMode, PermissionMatrix] = Field(
        default_factory=lambda: DEFAULT_MODE_PERMISSIONS.copy()
    )
    ...
    def validate_mode_permissions(self) -> "AutonomyPolicy":
        expected = {decision for decision in DecisionType}
        for mode, matrix in self.mode_permissions.items():
            missing = expected - set(matrix.keys())
            if missing:
