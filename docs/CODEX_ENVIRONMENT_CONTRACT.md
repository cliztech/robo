...
### Next.js Supabase runtime variables

Canonical variable names for Next.js runtime parity:

- `NEXT_PUBLIC_SUPABASE_URL`
- `NEXT_PUBLIC_SUPABASE_ANON_KEY`

Deprecated compatibility aliases (temporary):
...
- Configure canonical keys in all local/dev/staging/prod environments.
- Keep aliases only during the transition period.
- `src/lib/supabase/server.ts` currently logs a warning when alias fallback is used.
...
| `NEXT_PUBLIC_SUPABASE_URL` | Environment injection (`.env`, `.env.local`, secret manager) | `SUPABASE_URL` (deprecated, temporary) | Required for Supabase URL resolution in Next.js runtime. |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | Environment injection (`.env`, `.env.local`, secret manager) | `SUPABASE_ANON_KEY` (deprecated, temporary) | Required for Supabase anon key resolution in Next.js runtime. |
