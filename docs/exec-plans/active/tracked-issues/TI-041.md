...
- **Release readiness status:** blocked (awaiting TI-040 dependency evidence closure)
...
| `SMK-AUTHZ-01` | `pnpm test:security -- --case authz-role-deny` | Exit code `0` + `AUTHZ_DENIED_EXPECTED` + `HTTP_403_OBSERVED` + `CONTROL_TI040_ENCRYPTED_CONFIG_DENIED` + `CONTROL_TI040_HASH_EVIDENCE_PRESENT` (`artifacts/security/hashes/ti-040-config-before-after.sha256`) | Exit non-zero, missing markers, missing TI-040 hash evidence marker, or `AUTHZ_UNEXPECTED_ALLOW` |
...
| `SMK-PRIV-01` | `pnpm test:security -- --case privileged-action-block` | Exit code `0` + `PRIV_ACTION_BLOCKED` + `CONTROL_TI039_APPROVAL_ENFORCED` + `CONTROL_TI040_ENCRYPTED_CONFIG_DENIED` + `CONTROL_TI040_HASH_EVIDENCE_PRESENT` (`artifacts/security/hashes/ti-040-config-before-after.sha256`) | Exit non-zero, missing markers, `PRIV_ACTION_EXECUTED`, or missing TI-039/TI-040 control/hash markers |
...
- [ ] `DEP-TI041-04`: TI-040 encrypted-field controls and hash evidence are required in authorization-denial paths.
  - Blocker evidence: TI-040 dependency checkpoint `DEP-TI040-03` remains open due to missing high-risk keys in current config payloads, so TI-041 cannot be marked complete.
  - Evidence refs: `docs/exec-plans/active/tracked-issues/TI-040.md` (`DEP-TI040-03`); `artifacts/security/hashes/ti-040-config-before-after.sha256` (required but dependency is not yet executable).

### Dependency evidence snapshot (TI-040 gate linkage)

TI-041 can remain `implementation_status: complete` only while smoke scenarios and marker assertions continue to pass. `release_readiness_status` may move to `ready` only after TI-040 artifacts below exist and are verifiable:

- `TI040-ART-LOG`: `artifacts/security/logs/ti-040-config-encryption.log`
- `TI040-ART-REPORT`: `artifacts/security/reports/ti-040-high-risk-field-inventory.md`
- `TI040-ART-HASH`: `artifacts/security/hashes/ti-040-config-before-after.sha256`

Current snapshot (2026-03-03): all three TI-040 artifact IDs are pending in-repo, so TI-041 release gate remains blocked even though implementation is complete.

### Missing deliverables required to close TI-041

- `TI040-ART-LOG`: `artifacts/security/logs/ti-040-config-encryption.log` (not present).
- `TI040-ART-REPORT`: `artifacts/security/reports/ti-040-high-risk-field-inventory.md` (not present).
- `TI040-ART-HASH`: `artifacts/security/hashes/ti-040-config-before-after.sha256` (not present).

TI-041 remains reopened (`Status: Open`) until the three dependency deliverables above exist and are verifiable.
...
TI-041 can keep `implementation_status: complete` and advance to `release_readiness_status: ready` only if all are true on rerun:
...
Current state: **Blocked** until TI-040 publishes completed `DEP-TI040-01..03` evidence with usable encrypted-field target keys and hash artifacts.
