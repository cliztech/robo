...
### Dependency evidence snapshot (2026-03-03)

| Checkpoint | Status | Evidence link(s) | Downstream impact |
| --- | --- | --- | --- |
| `DEP-TI040-01` | Open | Pending artifact publication: `artifacts/security/reports/ti-040-high-risk-field-inventory.md` (`CHK-TI040-03`, TI-004 ticket linkage) | TI-041 release gate cannot claim encrypted-config dependency closure until `kid` provenance is traceable to TI-004 approvals. |
| `DEP-TI040-02` | Open | TI-039 schema contract defines required fields in `docs/exec-plans/active/tracked-issues/TI-039.md` and `docs/operations/artifacts.md`; TI-040 execution artifact `artifacts/security/hashes/ti-040-config-before-after.sha256` not yet present | TI-041 can run smoke implementation checks, but release-readiness remains blocked because control evidence cannot be reconciled to immutable audit export hashes. |
| `DEP-TI040-03` | Open | Field inventory currently exists only as planning targets in this document; verification command `rg -n "openai_api_key|tts_api_key|webhook_auth_token|stream_fallback_password|remote_ingest_secret" config/prompt_variables.json config/schedules.json` returned no matches in current repo state | Encryption rollout cannot proceed to `Complete`; TI-041 must remain in partial completion mode (implementation complete, release gate incomplete). |
