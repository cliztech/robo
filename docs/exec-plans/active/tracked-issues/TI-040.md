...
1. `artifacts/security/logs/ti-040-config-encryption.log`
   - Includes validation command output and envelope check result.
2. `artifacts/security/reports/ti-040-high-risk-field-inventory.md`
   - Includes key provenance (`kid`, creation ticket, rotation ticket, approver principal) and failure-handling decision.
3. `artifacts/security/hashes/ti-040-config-before-after.sha256`
   - Includes `before_hash_sha256` and `after_hash_sha256` records compatible with TI-039 export fields.

On decryption failure, operator flow is deterministic: restore latest known-good encrypted backup, re-run validation, regenerate before/after hash evidence, and append rollback reason to the report.
