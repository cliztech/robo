...
## 2024-07-12 - [Refactoring bash spawn]
**Vulnerability:** Shell evaluation of arguments via `bash -lc`
**Learning:** Using `bash -lc` with interpolated strings like `filepath` exposes the application to command injection.
**Prevention:** Call the executable directly (e.g. `ffmpeg` or `mkfifo`) with an argument array instead of routing through bash.
