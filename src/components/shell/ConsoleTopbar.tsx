...
export function ConsoleTopbar({ currentView, isOnAir, onToggleOnAir }: ConsoleTopbarProps) {
  ...
  const { activeSkinId, resolvedTheme } = useThemePreferences();
  ...
  return (
    <header className="h-11 skin-panel border-b flex items-center justify-between px-5 shrink-0 z-10 relative">
      <div className="flex items-center gap-4 min-w-0">
        <div className="flex items-center gap-2">
          <Disc3 size={11} className="text-[hsl(var(--color-deck-a))]/80" />
          <span className="text-[11px] font-black uppercase tracking-[0.2em] text-[hsl(var(--color-text))]">DGN-DJ Studio</span>
        </div>
      ...
      <div className="flex items-center gap-3">
        <div className={cn('hidden lg:flex h-2 w-16 rounded-full bg-gradient-to-r', previewClass)} data-theme={resolvedTheme} />

        <button className="relative p-1.5 text-[hsl(var(--color-text-muted))] hover:text-[hsl(var(--color-text))] transition-colors" aria-label="Open alerts">
