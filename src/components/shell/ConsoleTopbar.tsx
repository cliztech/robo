...
export function ConsoleTopbar({
  ...
  const activeView =
    CONSOLE_NAV_ITEMS.find((item) => item.view === currentView)?.label ??
    currentView;
  const {
    themeMode,
    activeSkinId,
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className={cn(
            ...
            isOnAir
              ? "skin-control skin-control-active pulse-ring"
              : "skin-control skin-control-inactive",
          )}
        >
        ...
        <div className="flex items-center gap-2 px-2 py-1 rounded skin-panel-muted">
          ...
          <span className="text-[9px] font-mono text-[hsl(var(--color-text-muted))] tabular-nums">
            06%
          </span>
        </div>
      </div>
