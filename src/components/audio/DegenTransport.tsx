...
export function DegenTransport({
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => setTelemetryStep((prev) => prev + 1), 80);
    return () => clearInterval(id);
  }, [isPlaying, telemetryTick]);

  const phase = typeof telemetryTick === 'number' ? telemetryTick : telemetryStep;
  const progress = progressOverride ?? telemetry?.transport.progress ?? transportTelemetry.progress;
  const elapsed = telemetry?.transport.elapsedSeconds ?? progress * (track.duration || 0);
  const remaining = useMemo(() => {
    if (telemetry) return telemetry.transport.remainingSeconds;
    return Math.max(0, (track.duration || 0) - elapsed);
  }, [elapsed, telemetry, track.duration]);

  const vuLeft = telemetry?.stereoLevels.leftLevel ?? Math.max(0.1, Math.min(1, transportTelemetry.vuLeft + Math.sin(phase / 5) * 0.08));
  const vuRight = telemetry?.stereoLevels.rightLevel ?? Math.max(0.1, Math.min(1, transportTelemetry.vuRight + Math.cos(phase / 6) * 0.08));
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1 px-3 shrink-0">
        <button type="button" onClick={() => setShuffle((v) => !v)} aria-label="Toggle shuffle" aria-pressed={shuffle} className={cn('p-1.5 rounded transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lime-400/70 focus-visible:ring-offset-2 focus-visible:ring-offset-black', shuffle ? 'text-deck-a' : 'text-zinc-600 hover:text-zinc-300')}>
          <Shuffle size={12} />
        </button>
        <button type="button" onClick={onPrev} aria-label="Previous track" className="p-1.5 text-zinc-400 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lime-400/70 focus-visible:ring-offset-2 focus-visible:ring-offset-black">
          <SkipBack size={14} />
        </button>
        <button
          type="button"
          ...
          className={cn(
            ...
            isPlaying ? 'bg-lime-400/90 text-black' : 'bg-zinc-700 text-white'
          )}
        >
          {isPlaying ? <Pause size={16} /> : <Play size={16} className="ml-0.5" />}
        </button>
        <button type="button" onClick={onNext} aria-label="Next track" className="p-1.5 text-zinc-400 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lime-400/70 focus-visible:ring-offset-2 focus-visible:ring-offset-black">
          <SkipForward size={14} />
        </button>
        <button type="button" onClick={() => setRepeat((v) => !v)} aria-label="Toggle repeat" aria-pressed={repeat} className={cn('p-1.5 rounded transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lime-400/70 focus-visible:ring-offset-2 focus-visible:ring-offset-black', repeat ? 'text-deck-b' : 'text-zinc-600 hover:text-zinc-300')}>
          <Repeat size={12} />
        </button>
      </div>

      <div className="flex-1 flex items-center gap-3 px-3 min-w-0 border-l border-white/[0.04] border-r border-white/[0.04]">
        <span className="text-[10px] font-mono text-zinc-500 tabular-nums w-9 text-right shrink-0">{formatTime(elapsed)}</span>
        <div className="flex-1 relative group" aria-hidden>
          <div className="h-[3px] rounded-full bg-white/[0.06] overflow-hidden">
            <div data-testid="transport-progress-fill" className="h-full rounded-full bg-[hsl(var(--color-deck-a))] transition-[width] duration-75" style={{ width: `${progress * 100}%` }} />
          </div>
        </div>
        <input
          ...
          onMouseUp={(e) => {
            const nextProgress = parseFloat((e.currentTarget as HTMLInputElement).value);
            onSeek?.(nextProgress);
      ...
      <div className="flex items-center gap-2 px-3 shrink-0 border-r border-white/[0.04]">
        <div className="flex flex-col items-center px-2 py-1 rounded bg-white/[0.02] border border-white/[0.05]">
        ...
        <div className="flex flex-col items-center justify-center px-1 gap-0.5">
          ...
          <span className="text-[8px] font-black uppercase tracking-wider text-zinc-600">On Air</span>
        </div>
      </div>

      <div className="px-3 shrink-0 border-r border-white/[0.04]">
        <DegenStereoMeter leftLevel={vuLeft} rightLevel={vuRight} leftPeak={peakLeft} rightPeak={peakRight} size="sm" showDb />
      </div>

      <div className="flex items-center gap-2 px-3 w-36 shrink-0">
        <button
          type="button"
          onClick={() => setIsMuted((v) => !v)}
          aria-label={isMuted ? 'Unmute output' : 'Mute output'}
          aria-pressed={isMuted}
          className={cn('p-1 rounded transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lime-400/70 focus-visible:ring-offset-2 focus-visible:ring-offset-black', isMuted ? 'text-red-400' : 'text-zinc-500 hover:text-zinc-300')}
        >
          <VolumeIcon size={14} />
        </button>
