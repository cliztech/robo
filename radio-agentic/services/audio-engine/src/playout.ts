...
export class PlayoutEngine {
  ...
  async startLoop() {
    if (!fs.existsSync(this.fifoPath)) {
      // Security: use direct executable to prevent shell operator injection
  ...
  private async playTrackToFifo(t: Track) {
    ...
    // Security: avoid shell invocation and string interpolation to prevent injection attacks
